    ...
    useEffect(() => {
        async function fetchProducts() {
            try {
                ...
                // Add parameters if there are filters or sorting options
                ...
                // Adjust to API response structure
                let filteredProducts = data.data || data || [];

                // If API already handles filtering and sorting on the server, no need to filter again on client
                // But if API doesn't support it, we still do it on the client
                ...
                const uniqueCategories = Array.from(
                    new Set(filteredProducts.map((product: Product) => {
                        ...
                        return null; // Skip if no category
                    }).filter(Boolean)) // Filter null values
                ).map((str: unknown) => typeof str === 'string' ? JSON.parse(str) : null).filter(Boolean); // Parse dan filter null lagi
            ...
            {loading ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {products.map((product) => (
                        <div key={product.id} className="card bg-base-100 shadow-md hover:shadow-xl transition-shadow">
